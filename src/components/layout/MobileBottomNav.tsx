...
export function MobileBottomNav({ showAdmin }: { showAdmin: boolean }) {
  ...
  const items: Item[] = [
    ...
    { href: "/shelves", label: "Étagères", icon: LayersIcon, match: (p) => isSubpath(p, "/shelves") },
    { href: "/search", label: "Recherche", icon: SearchIcon, match: (p) => isSubpath(p, "/search") },
    ...(showAdmin
      ? [{ href: "/admin/books", label: "Admin", icon: ShieldIcon, match: (p) => isSubpath(p, "/admin") }]
      : []),
  ];
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-50 border-t border-(--eleven-border-subtle) bg-background/90 backdrop-blur sm:hidden"
    >
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "bg-secondary text-foreground shadow-eleven-card" : "text-eleven-muted hover:text-foreground",
              )}
            >
