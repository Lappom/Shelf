...
export function ReaderTocList({ items, emptyLabel, onNavigate, listClassName }: Props) {
  if (!items.length) {
    return (
      <div className="text-eleven-muted eleven-body-airy px-2 py-2 text-sm">{emptyLabel}</div>
    );
  }
  return (
    <div className={cn("px-2 pb-3", listClassName)}>
      {items.map((it, index) => (
        <button
          ...
          type="button"
          className={cn(
            "hover:bg-muted/80 eleven-body-airy w-full rounded-eleven-warm px-2 py-2 text-left text-sm transition-colors",
